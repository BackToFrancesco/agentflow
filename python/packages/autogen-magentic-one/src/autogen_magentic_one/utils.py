...
def create_completion_client_from_env(env: Dict[str, str] | None = None, **kwargs: Any) -> ChatCompletionClient:
    ...
    _provider = env.get(ENVIRON_KEY_CHAT_COMPLETION_PROVIDER, "azure").lower().strip()

    # Instantiate the correct client
    if _provider == "openai":
        print("Using OpenAI ChatCompletionClient")
        return OpenAIChatCompletionClient(**_kwargs)  # type: ignore
    elif _provider == "azure":
        print("Using Azure OpenAI ChatCompletionClient")
...
class LogHandler(logging.FileHandler):
    def __init__(self, filename: str = "log.jsonl") -> None:
        ...
        # Now initialize the FileHandler with the cleared file
        super().__init__(filename, mode='a')  # 'a' for append mode
